...
interface Tools {
  ...
  grep<T>(
    ...
    pred?: ArrayCallback$1<T, boolean>
  ): T[];
  grep<T>(
    arr: Record<string, T> | null | undefined,
    pred?: ObjCallback$1<T, boolean>
  ): T[];
  ...
  each<T>(
    arr: ArrayLike<T> | null | undefined,
    cb: ArrayCallback$1<T, void | boolean>,
    scope?: any
  ): boolean;
  each<T>(
    obj: Record<string, T> | null | undefined,
    cb: ObjCallback$1<T, void | boolean>,
    scope?: any
  ): boolean;
  map<T, R>(
    arr: ArrayLike<T> | null | undefined,
    cb: ArrayCallback$1<T, R>
  ): R[];
  map<T, R>(
    obj: Record<string, T> | null | undefined,
    cb: ObjCallback$1<T, R>
  ): R[];
...
type PublicDialog_d_DialogInstanceApi<_0 extends DialogData> =
  DialogInstanceApi<_0>;
...
interface DOMUtils {
  ...
  getParent<K extends keyof HTMLElementTagNameMap>(
    node: string | Node,
    selector: K,
    root?: Node
  ): HTMLElementTagNameMap[K] | null;
  getParent<T extends HTMLElement>(
    node: string | Node,
    selector: (node: HTMLElement) => node is T,
    root?: Node
  ): T | null;
  getParent<T extends Element = Element>(
    node: string | Node,
    selector?: string | ((node: HTMLElement) => boolean | void),
    root?: Node
  ): T | null;
  getParents<K extends keyof HTMLElementTagNameMap>(
    elm: string | Node,
    selector: K,
    root?: Node,
    collect?: boolean
  ): Array<HTMLElementTagNameMap[K]>;
  getParents<T extends HTMLElement>(
    node: string | Node,
    selector: (node: HTMLElement) => node is T,
    root?: Node
  ): T[];
  getParents<T extends Element = Element>(
    elm: string | Node,
    selector?: string | ((node: HTMLElement) => boolean | void),
    root?: Node,
    collect?: boolean
  ...
  select<T extends HTMLElement = HTMLElement>(
    selector: string,
    scope?: string | Node
  ): T[];
  ...
  insertAfter<T extends Node>(
    node: RunArguments<T>,
    reference: string | Node
  ): false | T;
  replace<T extends Node>(
    newElm: Node,
    oldElm: T | T[],
    keepChildren?: boolean
  ): T;
  ...
  rename<K extends keyof HTMLElementTagNameMap>(
    elm: Element,
    name: K
...
declare namespace RangeUtils {
  const compareRanges: (
    rng1: RangeLikeObject,
    rng2: RangeLikeObject
  ) => boolean;
